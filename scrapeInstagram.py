...
FIELD_NAMES = ['instagram_handle', 'instagram_url', 'timestamp', 'number_of_followers']

PREFIX = 'https://instagram.com/'


# This class represents the spider which will be run by Scrapy to scrape the number of followers from Instagram pages.
class InstagramScraper(scrapy.Spider):
	name = 'Instagram Scraper'

	# This variable is used by Scrapy to begin crawling.
	start_urls = []

	# This dictionary holds the mapping of the URLs to Instagram handles, which is used when populating the output file.
	url_map = {}
	...
	def parse(self, response):
		...
		p = re.compile('.*"followed_by"\s*:\s*{\s*"count"\s*:\s*(\d+)\s*}')
